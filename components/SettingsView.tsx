  ...
  return (
    <div className="max-w-4xl mx-auto animate-fade-in pb-12">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
        <div className="flex items-center justify-between mb-6">
            <h3 className="text-lg font-bold text-slate-800 flex items-center">
                <i className="fas fa-database mr-2 text-emerald-500"></i>
                CMS連携設定 (Supabase)
            </h3>
            <div className="flex items-center gap-2 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-200">
                <span className="text-xs font-bold text-slate-500 uppercase mr-2">Auto Post</span>
                <button 
                    onClick={() => updateSupabase('autoPost', !localSettings.supabase.autoPost)}
                    className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${localSettings.supabase.autoPost ? 'bg-blue-500' : 'bg-slate-300'}`}
                >
                    <span className={`inline-block h-3 w-3 transform rounded-full bg-white transition-transform ${localSettings.supabase.autoPost ? 'translate-x-5' : 'translate-x-1'}`} />
                </button>
            </div>
        </div>
        ...
        <div className="space-y-4">
            <div>
                ...
                <div className="flex items-center gap-2">
                    <input 
                        ...
                        readOnly
                        placeholder="(環境変数 SUPABASE_URL が設定されていません)"
                        className="w-full bg-slate-100 border border-slate-300 rounded px-3 py-2 text-sm text-slate-500 font-mono outline-none cursor-not-allowed"
                    />
                    <span className="hidden md:inline-block text-[10px] px-2 py-1 bg-slate-200 text-slate-500 rounded whitespace-nowrap border border-slate-300">Managed</span>
                </div>
            </div>
            <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Service Anon Key</label>
                <div className="flex items-center gap-2">
                    <input 
                        type="password" 
                        value={localSettings.supabase.anonKey ? "********************************" : ""}
                        readOnly
                        placeholder="(環境変数 SUPABASE_ANON_KEY が設定されていません)"
                        className="w-full bg-slate-100 border border-slate-300 rounded px-3 py-2 text-sm text-slate-500 font-mono outline-none cursor-not-allowed"
                    />
                    <span className="hidden md:inline-block text-[10px] px-2 py-1 bg-slate-200 text-slate-500 rounded whitespace-nowrap border border-slate-300">Managed</span>
                </div>
            ...
            <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Author ID (UUID)</label>
                 <div className="flex items-center gap-2">
                    <input 
                        type="text" 
                        value={localSettings.supabase.authorId || ''}
                        readOnly
                        placeholder="(環境変数 SUPABASE_AUTHOR_ID が設定されていません)"
                        className="w-full bg-slate-100 border border-slate-300 rounded px-3 py-2 text-sm text-slate-500 font-mono outline-none cursor-not-allowed"
                    />
                    <span className="hidden md:inline-block text-[10px] px-2 py-1 bg-slate-200 text-slate-500 rounded whitespace-nowrap border border-slate-300">Managed</span>
                </div>
        ...
        <div className="mt-8 flex justify-end">
            <button 
                onClick={handleSave}
                className={`
                    px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg shadow-sm transition-all flex items-center gap-2
            ...
            >
                {isSaving ? (
                    <>
                        <i className="fas fa-circle-notch fa-spin"></i> 保存中...
                    </>
                ) : (
                    <>
