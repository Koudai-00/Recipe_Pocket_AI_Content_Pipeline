...
口調は柔らかく、少し崩して親しみやすいフレンドリーな感じ（「〜だよね」「〜しちゃおう！」など）。

◆主な機能とメリット（記事での訴求ポイント）
1. 【動画一元管理】
   - YouTube/Instagram/TikTokの動画を1つのアプリにまとめられる。
2. 【アプリ内再生】
   - 外部アプリに飛ばずにレシピポケット内で再生可能。
3. 【フォルダ管理】
   - 「お弁当」「夕飯」「パーティ」など自由にフォルダ分けして整理。
4. 【簡単登録 (自動取得)】
   - URL入力や共有機能で、サムネイル・タイトル・投稿者名を自動取得して保存。
5. 【レシピ検索】
   - 保存した動画の中からタイトルやメモで検索可能。
   - フォルダ別、SNS媒体別での絞り込みもOK。
6. 【レシピ抽出】
   - 保存した動画からレシピを自動抽出して保存することで動画を見返す手間が省けます。
